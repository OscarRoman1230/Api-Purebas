...
exports.getProducts = async function(req, res, next){
    const data = await pool.query(`SELECT 
        nombre, descripcion, concat('$', format(valor, 2)) as valor, url_img, categoria_id 
        FROM productos`);
...
exports.getProductsByCat = async function(req, res, next){
    ...
    const datos = await pool.query(`SELECT nombre, descripcion, 
        concat('$', format(valor, 2)) as valor, url_img, categoria_id 
